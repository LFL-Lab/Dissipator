...
if resFreq is res1:
    ...
    # automatic mixer calibration, 8/24/22
    # rr_LO = int(6.75e9) # don't forget to change this on the signal generator
    # rr_imbalance = [0.15000, -0.15000]
    # rrI_analog_out = {"offset":  0.09214} # DC offset on I #-0.025
    # rrQ_analog_out = {"offset":  0.02678} # DC offset on Q
...
if qbFreq is qb_A:
    ...
    # qubit_imbalance = [0.0628, -0.1088]
    # qb_mixer_IQ_imbalance = IQ_imbalance(*qubit_imbalance)
    # qbI_analog_out = {"offset": -0.015517} 
    # qbQ_analog_out = {"offset": -0.015517} 
    
    # lower sideband
    qb_LO = int(3.50e9)  # don't forget to change this on the signal generator
    qubit_imbalance = [0.0672, -0.0569]
    qb_mixer_IQ_imbalance = IQ_imbalance(*qubit_imbalance)
    qbI_analog_out = {"offset": -0.01897} 
    qbQ_analog_out = {"offset": -0.01552} 
    
    pi_half_len = 16 # needs to be a multiple of 4, in ns unit
    ...
    qubit_imbalance = [0.038, -0.044]
    qb_mixer_IQ_imbalance = IQ_imbalance(*qubit_imbalance)
    qbI_analog_out = {"offset": -0.01915} # need to calibrate; remove note when calibrated
    qbQ_analog_out = {"offset": -0.0052} # need to calibrate; remove note when calibrated
    
    pi_half_len = 32 # needs to be a multiple of 4, in ns unit
    pi_len = 2 * pi_half_len
    pi_amp =   0.3597 / 100 #0.274
    
    
elif qbFreq is qubit2_biased:
    
    # upper sideband
    qb_LO = int(5.3e9)  # don't forget to change this on the signal generator
    qubit_imbalance = [-0.042, -0.035]
    qb_mixer_IQ_imbalance = IQ_imbalance(*qubit_imbalance)
    qbI_analog_out = {"offset": -0.01495} # need to calibrate; remove note when calibrated
    qbQ_analog_out = {"offset": -0.0022} # need to calibrate; remove note when calibrated
