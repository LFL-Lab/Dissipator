...
def timerabi(t0 = 4,         # minimum pulse duration in nanoseconds
              tf = 1.0e3,    # maximum pulse duration in nanoseconds
              dt = 4,        # step of sweep in nanoseconds
    ...
    times = np.arange(t_min, t_max + step/2, step) # in clockcycles
...
def powerrabi(a_min = 0.01,    # minimum amp_q scaling
               ...
               pulse_len = 100,  # pulse length in nanoseconds
               makeplots = True,
    ...
    with program() as power_rabi:
        ...
        with for_(n, 0, n < n_avg, n + 1):
            ...
            with for_(a, a_min, a < a_max + da/2, a + da):  # Sweep pulse duration
    ...
    figname = f'{foldername}/figures/power_rabi_pulselen_{pulse_len}ns_n_{n_avg}_da{da}_amax{a_max}'
    datname = f'{foldername}/data/power_rabi_pulselen_{pulse_len}ns_n_{n_avg}_da{da}_amax{a_max}'
    ...
    if makeplots:
        ...
        plot_rabi(amps, Q, title = f"pulse length {pulse_len} ns, " )
        plt.savefig(figname + "_Q.png")
        plt.close()
