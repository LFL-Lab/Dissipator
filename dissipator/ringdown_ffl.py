...
def sweep_powers(qb, 
    ...
    if 'filename' in kwargs.keys():
        ...
        filename = f'ringdown_sweep{element}Powers_flux={flux*1e6}uA_fflFreq={(qb.pars["ffl_freq"])/1e9:.2f}GHz_DA={rr_atten}dB_fDA={qb.pars["ffl_atten"]}dB_rrLen={rrLen}clks_amp_r_scale={amp_r_scale}=navg={n_avg}_checkMixer={str(check_mixers)}'
        index = get_index_for_filename(saveDir, filename)
        mode = 'w'
    with h5py.File(f'{saveDir}\\{filename}_{index}.h5',mode) as hf:
        ...
        if bcheckDriveOff:
            dataDict, fig = measure_ringdown_drive_off(qb,tmax=1e3, dt=16, n_avg=n_avg, amp_r_scale = amp_r_scale)
        ...
        for j,value in enumerate(var_scales):
            if element == 'rr':
                ...
                dataDict, fig = measure_ringdown_drive_on(qb,amp_ffl_scale=value, n_avg=n_avg, dt = 4, tmax=0.4e3,amp_r_scale=amp_r_scale)
            save_datadict_to_fgroup(g_subdata, f'{element}_scale = {value}', dataDict)
...
def sweep_freq_power(qb, sa = None,  
    ...
    with h5py.File(f'{saveDir}\\{filename}_{index}.h5',mode) as hf:
        ...
        for freq in tqdm(freqs_list):
            ...
            # do res spec
            inst.turn_off_ffl_drive()
            I, Q, freqs, job = qb.resonator_spec(f_LO=qb.pars['rr_LO'],atten=qb.pars['rr_atten'],IF_min=35e6,IF_max=65e6,df=0.1e6,n_avg=n_avg,savedata=True, fit=False,fc=qb.pars['rr_freq'])
            # fc,fwhm = pf.fit_res(freqs,np.abs(I+1j*Q))
            # qb.update_value('rr_freq', fc)
            dataDict = {'I': I,
                        'Q': Q,
            ...
            if bOptimizeFFLMixer:
                ...
                if ffl_mixer_data['ffl_lo_leakage'] > -70:
