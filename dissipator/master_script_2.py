...
qb = dissipator('diss08_11a_5p9',device_name='diss08_11a_5p9')
sa =  inst.init_sa()

'''Update important parameters'''
qb.update_value('rr_freq', 5.9722e9)
qb.update_value('rr_LO', 5.75e9)
qb.update_value('rr_IF', qb.pars['rr_freq'] -qb.pars['rr_LO'] )
qb.update_value('rr_atten', 35)
qb.update_value('diss_freq', 7.97e9)
qb.update_value('qubit_freq', 4.218e9)
qb.update_value('qubit_LO', 4.05e9)
...
qb_lo_leakage = qb.get_power(sa, freq=qb.pars['qubit_LO'],reference=-10,config=True,plot=True)
...
qb.resonator_spec(f_LO=qb.pars['rr_LO'],atten=33,IF_min=220e6,IF_max=224e6,df=50e3,n_avg=5000,savedata=True)

#%% punchout
I, Q, freq_arr, job = qb.punchout(IF_min=20e6,IF_max=70e6,df=0.1e6, atten_range=[0,50], atten_step=2, n_avg=2000)
#%% qubit spectroscopy
qb.update_value('rr_atten', 35)
I,Q,freqs,job = qb.run_scan(df = 400e3, n_avg = 5000, element='qubit', chunksize = 250e6,  lo_min = 3e9, lo_max = 5e9, amp_q_scaling = 1, saturation_dur = 30e3, showprogress=True, res_ringdown_time = int(40e3), check_mixers=False)
...
amps, I, Q, job, period = qb.power_rabi(a_min = 0.01, a_max = 1., da = 0.005,  n_avg = 10000,fit = True, plot = True, detuning = 0e6, check_mixers = False)

qb.update_value('pi_amp', period)
...
t_arr, I, Q, job, pi2width = qb.pulse_exp(exp='rabi',n_avg = 10000, tmin = 32,tmax = 240, dt = 4, amp_q_scaling = 1, fit = True, plot = True, detuning = 0e6, check_mixers=False)
qb.update_value('pi_half_len', 16)
qb.update_value('pi_len', 32)
# plot rabi
x_vector = t_arr
...
t_arr, I, Q, job, fitted_pars = qb.pulse_exp(exp='T1', check_mixers = False, n_avg=15000,dt=256,tmin=0,tmax=60e3,detuning=0e3)
t_arr, I, Q, job, fitted_pars = qb.pulse_exp(exp='ramsey', check_mixers = False, n_avg=80000,dt=256,tmin=0,tmax=10e3,detuning=0e3)


#%% Echo
qb.pulse_exp(exp = 'echo', n_avg = 15000, tmin = 40, tmax = 20e3, dt = 128, fit=True, check_mixers=False)
...
#for freq in [3.58e9]:
for detuning in np.linspace(-20,60,20)*1e6:
        #qb.update_value('qubit_freq', freq)
        #qb.update_value('qubit_IF',100e6)
        #qb.update_value('qubit_LO', qb.pars['qubit_freq']-qb.pars['qubit_IF'])
        #inst.set_qb_LO(qb.pars['qubit_LO'])
        #t_arr, I, Q, job, pi2width = qb.pulse_exp(exp='rabi',n_avg = 20000, tmin = 0,tmax = 400, dt = 4, amp_q_scaling = 1, fit = True, plot = True,detuning = detuning, check_mixers=False)
    amps, I, Q, job, period = qb.power_rabi(a_min = 0.01, a_max = 1., da = 0.01,  n_avg = 4000,fit = True, plot = True, detuning = detuning, check_mixers = False)