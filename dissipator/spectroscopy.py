...
with h5py.File(f'{saveDir}\\{filename}_{index}.h5','w') as hf:
    ...
    if bCalibrateRo:
        ...
        I0, Q0, freqs, job = qb.resonator_spec(f_LO=qb.pars['rr_LO'],atten=qb.pars['rr_atten'],IF_min=45e6,IF_max=55e6,df=0.1e6,n_avg=n_avg,savedata=True,fit=False)
    ...
    for flux in tqdm(flux_list):
        ...
        if bCalibrateRo:
            I, Q, freqs, job = qb.resonator_spec(f_LO=qb.pars['rr_LO'],atten=qb.pars['rr_atten'],IF_min=45e6,IF_max=55e6,df=0.1e6,n_avg=n_avg,savedata=True,fit=False)
        ...
        I,Q,freqs,job = qb.run_scan(df = 5e6, n_avg = n_avg, element='qubit', chunksize = 300e6,  lo_min = 8e9, lo_max = 11e9, amp_q_scaling = amp_q_scaling, saturation_dur = 20e3, showprogress=True, res_ringdown_time = int(4e3), check_mixers=False)
        dataDict = {'metadata': {'flux': flux,
                                 'rr_freq': qb.pars['rr_freq'],
                                 'rr_LO': qb.pars['rr_LO'],
                                 'rr_IF': qb.pars['rr_IF'],
